...
export const gameStateModules: GameStateModule[] = [
...

  {
    ...
    whyItMatters:
      "England has, at various points in recent tournament history, been described by commentators as dropping too deep for long stretches of matches even when defending is not strictly required by the scoreline, ceding control of games it had periods of dominance in. The question is when the team collectively decides to defend a lower line versus stepping up together, a matter of coordination and confidence rather than of individual defensive quality.",
    validResponses: [
      {
...

  {
    ...
    coachingExplanation:
      "Chasing a goal is a genuine tactical challenge, not simply a matter of committing more players forward: it requires a rehearsed structure for how many players go forward, who covers the space they leave, and what the rest-defence shape looks like if possession is lost in an advanced position. Coaching this well means preparing specific patterns in training, for example an auxiliary attacking substitute with a covering midfielder holding position, rather than leaving the team to improvise an unstructured, high-risk push that leaves it exposed to a fast break.",
    whyItMatters:
